    ...
    *,
    ...
    for contact in device.contacts:
        for i in layer_indices_dict[contact.name]:
            ...
            if not is_contact:
                for c in ('a', 'b'):
                    dependent_models.append(trafos.get_dx_model(
            ...
            if not is_out_contact:
                dependent_models.append(MultiModel(
                    lambda qs, i=i, contact=contact:
                        trafos.factors_trafo(qs, i, contact),
                ...
                for grid_name in boundaries_in + bulks + boundaries_out:
                    dependent_models.append(MultiModel(
                        lambda qs, contact=contact, grid_name=grid_name, i=i:
                            trafos.add_coeffs(qs, contact, grid_name, i),
                        f'coeffs{i}',
                    ))
            ...
            if not is_contact:
                for c in ('a', 'b'):
                    for grid_name in boundaries_in:
                        dependent_models.append(model.get_multi_model(
                            model.FunctionModel(
    ...
    for i in range(1, N + 1):
        ...
        for contact in device.contacts:
            ...
            dependent_models.append(model.MultiModel(
                loss.j_loss_trafo,
